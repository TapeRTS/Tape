    ...
    */
    public long getLastModifiedNewestCss() {
        ...
        for (String str : getActiveDocumentCss()) {
            ?? fileLastModified = getFileLastModified(str);
            if (fileLastModified > c) {
                c = fileLastModified;
            }
        }
        return c;
    ...
    public int CompileGeometry(float[] fArr, float[] fArr2, int[] iArr, int[] iArr2, int i) {
        ...
        while (true) {
            if (i3 >= this.compiledGeometryList.size()) {
                break;
            } else if (this.compiledGeometryList.get(i3) == null) {
                compiledGeometry.f325id = i3;
        ...
        if (!z2) {
            compiledGeometry.f325id = this.compiledGeometryList.size();
            this.compiledGeometryList.add(compiledGeometry);
        }
        return compiledGeometry.f325id;
    }
