...
public class OpenALAudioDevice implements AudioDevice {
    ...
    public void writeSamples(short[] sArr, int i, int i2) {
        ...
        for (int i4 = i; i4 < min; i4++) {
            short s = sArr[i4];
            int i5 = i3 + 1;
            this.bytes[i3] = (byte) (s & 255);
            i3 = i5 + 1;
            this.bytes[i5] = (byte) ((s >> 8) & 255);
        }
    ...
    public void writeSamples(float[] fArr, int i, int i2) {
        ...
        int i3 = 0;
        for (int i4 = i; i4 < min; i4++) {
            int a = (int) (C0876m.m292a(fArr[i4], -1.0f, 1.0f) * 32767.0f);
            int i5 = i3 + 1;
            this.bytes[i3] = (byte) (a & 255);
            i3 = i5 + 1;
            this.bytes[i5] = (byte) ((a >> 8) & 255);
        }
        writeSamples(this.bytes, 0, i2 * 2);
    }
            ...
            if (this.sourceID != -1) {
                if (this.buffers == null) {
                    ...
                    if (AL10.alGetError() != 0) {
                        throw new C0866c("Unabe to allocate audio buffers.");
                    }
                }
